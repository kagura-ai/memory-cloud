      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">{t('lastNDays', { days })}</p>
            ...
            <div className="space-y-1">
              ...
              <p className="text-2xl font-bold">{stats.resource_ingest?.avg_per_day?.toFixed(1) ?? '0.0'}</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">{t('activeTokens')}</p>
          ...
          {/* Timeline Chart */}
          {stats.resource_ingest?.timeline && stats.resource_ingest.timeline.length > 0 && (
            <div className="space-y-2">
              <h4 className="font-semibold text-sm">{t('eventsTimeline')}</h4>
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={stats.resource_ingest.timeline} aria-label={t('eventsTimeline')}>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">{t('anonymousSearches')}</p>
              <p className="text-2xl font-bold text-orange-600">
                {stats.public_search?.anonymous?.toLocaleString() ?? 0}
              </p>
            </div>
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">{t('authenticatedSearches')}</p>
              <p className="text-2xl font-bold text-blue-600">
          ...
          {stats.public_search?.timeline && stats.public_search.timeline.length > 0 && (
            <div className="space-y-2">
              <h4 className="font-semibold text-sm">{t('searchesTimeline')}</h4>
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={stats.public_search.timeline} aria-label={t('searchesTimeline')}>
                  <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
                  <XAxis
                    dataKey="date"
                    tickFormatter={formatDate}
