/**
 * Resource Tokens Page
 *
 * Issue #242 - Resource Token Management UI
 *
 * Features:
 * - Display resource tokens in a table
 * - Create new tokens (Owner only)
 * - Revoke tokens (Owner only)
 * - One-time plaintext display on creation
 */
...
export default function ResourceTokensPage() {
  ...
  const [tokenToRevoke, setTokenToRevoke] = useState<ResourceToken | null>(null);
  const [tokenToEdit, setTokenToEdit] = useState<ResourceToken | null>(null);
  const [revoking, setRevoking] = useState(false);
  const [editing, setEditing] = useState(false);
  const [editDescriptionInput, setEditDescriptionInput] = useState('');  // Input value
  const [editQuotaInput, setEditQuotaInput] = useState('');  // Input value
  ...
  }, [currentWorkspace, isOwner, router]);
  ...
  const loadTokens = async () => {
    try {
      ...
      // Issue #264: Handle paginated response
      ...
      // Issue #272 M-2: Use structured error codes instead of string matching
      // Normal empty state: RES-001 (Resource not found) with 404 status
      const isNormalEmptyState = err?.error === 'RES-001' && err?.status === 404;
      ...
      if (!isNormalEmptyState) {
        ...
        toast({
          title: tCommon('error'),
          description: err?.message || t('loadError'),
          variant: 'destructive',
        });
      }
  ...
  const handleRevokeConfirm = async () => {
    ...
    try {
      ...
      toast({
        title: tCommon('success'),
        description: t('revokeSuccess', { resourceId: tokenToRevoke.resource_id }),
      });
  ...
  return (
    <PageContainer>
      ...
      <FeatureGuide storageKey="resource-tokens" title={t('featureGuide.title')}>
              ...
              <div>
                ...
                <div className="bg-white dark:bg-gray-800 border border-blue-200 dark:border-blue-700 rounded p-3 space-y-2">
                  <div className="space-y-1 text-xs">
                    <p className="text-green-700 dark:text-green-300">{t('guide.dataFeature1')}</p>
                    <p className="text-green-700 dark:text-green-300">{t('guide.dataFeature2')}</p>
                    <p className="text-green-700 dark:text-green-300">{t('guide.dataFeature3')}</p>
                    <p className="text-green-700 dark:text-green-300">{t('guide.dataFeature4')}</p>
                  </div>
                  <div className="border-t border-blue-200 dark:border-blue-700 pt-2 space-y-1 text-xs">
                    <p className="text-amber-700 dark:text-amber-300">{t('guide.dataLimitation1')}</p>
                    <p className="text-amber-700 dark:text-amber-300">{t('guide.dataLimitation2')}</p>
                    <p className="text-amber-700 dark:text-amber-300">{t('guide.dataLimitation3')}</p>
                  </div>
                ...
                <div className="bg-white dark:bg-gray-800 border border-blue-200 dark:border-blue-700 rounded p-3 space-y-2">
                  <ul className="text-xs text-gray-700 dark:text-gray-300 space-y-1.5">
                    <li className="flex items-start gap-2">
                      ...
                      <span><strong>{t('guide.n1Rule1')}</strong></span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-blue-600 dark:text-blue-400">•</span>
                      <span><strong>{t('guide.n1Rule2')}</strong></span>
                    </li>
                  </ul>
                  <div className="mt-3 p-2 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-700 rounded">
                    ...
                    <div className="text-xs text-blue-800 dark:text-blue-200 space-y-1 ml-2">
                      ...
                      <p className="mt-2 pt-2 border-t border-blue-200 dark:border-blue-700 text-blue-700 dark:text-blue-300">
                        {t('guide.result1', { resourceId: 'products' })}<br />
                        {t('guide.result2')}
                      </p>
                    </div>
                  </div>
  ...
  "payload": {
    ...
    "price": 1000
                    ...
                    <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-blue-700 dark:text-blue-300">
                      <span>
                        <code className="bg-blue-100 dark:bg-blue-900 px-1.5 py-0.5 rounded">op</code>: {t('guide.paramOp')}
                      </span>
                      <span>
                        <code className="bg-blue-100 dark:bg-blue-900 px-1.5 py-0.5 rounded">doc_id</code>: {t('guide.paramDocId')}
                      </span>
                      <span>
                        <code className="bg-blue-100 dark:bg-blue-900 px-1.5 py-0.5 rounded">version</code>: {t('guide.paramVersion')}
                      </span>
                      <span>
                        <code className="bg-blue-100 dark:bg-blue-900 px-1.5 py-0.5 rounded">payload</code>: {t('guide.paramPayload')}
                      </span>
                    </div>
      ...
      <Section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-600 dark:text-slate-400">{t('activeTokens')}</p>
                <p className="text-3xl font-bold text-slate-900 dark:text-white mt-1">
                  {tokens.filter(t => t.status === 'active').length}
                  <span className="text-lg text-slate-400">
                    {' '}/ {currentWorkspace?.plan_name === 'pro' ? 30 : currentWorkspace?.plan_name === 'basic' ? 3 : 0}
                  </span>
                </p>
              </div>
              {tokens.filter(t => t.status === 'revoked').length > 0 && (
                <div className="text-right">
                  <p className="text-xs text-slate-500">{t('revokedCount', { count: tokens.filter(t => t.status === 'revoked').length })}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg">
            <p className="text-sm text-slate-600 dark:text-slate-400">{t('quotaUsage')}</p>
            <p className="text-3xl font-bold text-slate-900 dark:text-white mt-1">
              {tokens
                .filter(t => t.status === 'active')
                .reduce((sum, t) => sum + t.quota_events_per_hour, 0)
                .toLocaleString()}
              <span className="text-sm text-slate-400 ml-2">{t('eventsPerHour')}</span>
            </p>
            <p className="text-xs text-slate-500 mt-1">
              {(() => {
                const planName = (currentWorkspace?.plan_name || 'free') as 'free' | 'basic' | 'pro';
                const maxQuota = getMaxQuotaCapacity(planName);
                const currentQuota = tokens.filter(t => t.status === 'active').reduce((sum, t) => sum + t.quota_events_per_hour, 0);
                const percentage = maxQuota > 0 ? ((currentQuota / maxQuota) * 100).toFixed(1) : 0;
                return t('maxCapacity', { percentage, max: maxQuota.toLocaleString() });
              })()}
            </p>
          </div>
        </div>
        ...
        <ResourceTokensTable
          tokens={showRevoked ? tokens : tokens.filter(t => t.status === 'active')}
          contexts={contexts}
          loading={loading}
          ...
          <div className="mt-6 flex items-center justify-between border-t pt-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm px-3">
                {t('pageInfo', { current: currentPage, total: Math.ceil(totalTokens / limit) })}
              </span>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setCurrentPage((p) => p + 1)}
                disabled={currentPage >= Math.ceil(totalTokens / limit) || loading}
              >
                {tCommon('next')}
              </Button>
            </div>
        ...
        <AlertDialog open={showEditDialog} onOpenChange={setShowEditDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {t('editDialog.description', { resourceId: tokenToEdit.resource_id })}
              </AlertDialogDescription>
            </AlertDialogHeader>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  max={(() => {
                    ...
                    const maxTokens = currentWorkspace?.plan_name === 'pro' ? 30 : currentWorkspace?.plan_name === 'basic' ? 3 : 0;
                    const maxTotalQuota = maxTokens * maxPerToken;
                    const usedByOthers = tokens
                      .filter(t => t.status === 'active' && t.id !== tokenToEdit.id)
                      .reduce((sum, t) => sum + t.quota_events_per_hour, 0);
                    return Math.min(maxTotalQuota - usedByOthers, maxPerToken);
                  })()}
            ...
            <AlertDialogFooter>
              <AlertDialogCancel disabled={editing}>{t('editDialog.cancel')}</AlertDialogCancel>
              <AlertDialogAction onClick={handleEditConfirm} disabled={editing}>
                {editing && <span className="inline-block mr-2 h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent"></span>}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong className="font-semibold">{tokenToRevoke?.resource_id}</strong>?
              {tokenToRevoke && (
                <div className="p-2 bg-blue-50 dark:bg-blue-900/20 rounded text-sm mt-2 mb-2">
                  <p className="text-blue-900 dark:text-blue-100">
                    💡 This will free up <strong>{tokenToRevoke.quota_events_per_hour.toLocaleString()} {t('eventsPerHour')}</strong> quota.
                  </p>
                </div>
              )}
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={revoking}>{t('revokeDialog.cancel')}</AlertDialogCancel>
            <AlertDialogAction
              onClick={handleRevokeConfirm}
              disabled={revoking}
              className="bg-red-600 hover:bg-red-700"
            >
              {revoking ? t('revokeDialog.revoking') : t('revokeDialog.revoke')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
