...
- **`memory.kagura-ai.com` 本番状態**: **2026-05-17 に `True` 化済み**（`.env.prod` に `ENFORCE_WORKSPACE_CAP=true` を追記 → 当時の active service を service 名指定で再作成: `docker compose -f docker-compose.prod.yml --env-file .env.prod up -d --no-deps --force-recreate api-blue`）。**サービス名を明示しない `docker compose up -d --force-recreate` は postgres/redis/qdrant/web/caddy/api-* を全部 recreate してしまうため絶対に使わない** — 必ず active 色を指定する。flip 時点で 7 user 全員 `workspace_slot_bonus=0` / 所有 workspace 最大 1 個だったため、grandfathering 越えの false-positive denial リスクは 0 と DB で事前確認済み。flip 直後に operator 自身が新規 workspace 作成を試して `Workspace limit reached. You currently own 1 workspace(s) (cap: 1).` を実体験して deny 動作を verify。
