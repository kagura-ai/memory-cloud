...
Descriptions, summaries and examples are not part of either snapshot, so
wording changes need no regeneration. A new route, parameter, field, status
code or enum value does: run the command from `backend/`, commit the fixture
with the change, and review its diff like code. When a FastAPI or pydantic
upgrade moves the OpenAPI fixture everywhere at once, judge the change by the
failure message, which lists the paths and schemas that differ, rather than
by the raw fixture diff.
