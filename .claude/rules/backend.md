...
**Storage (DB) — mixed-awareness, predominantly naive UTC.** Most columns are `TIMESTAMP WITHOUT TIME ZONE` (naive UTC by convention); **14 columns across 7 models** are `TIMESTAMP WITH TIME ZONE` (aware): `Context.last_used_at` (auth.py), 2 timestamps in `config.py` (declared with `TIMESTAMP(timezone=True)` rather than `DateTime(timezone=True)`), and timestamps in `bm25_drift.py`, `erasure.py`, `hub_tag.py`, `neural.py`, and `terms.py` (`TermsAcceptance.accepted_at`, #1665). **Always check the model's `mapped_column(...)` declaration before constructing comparison sentinels or default values** — and grep both `DateTime(timezone=True)` and `TIMESTAMP(timezone=True)` since the codebase mixes both styles. Never assume naive across the board.
