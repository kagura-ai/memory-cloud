...
- **Codex CLI:** ask the kagura-memory skill to "log in to Kagura Memory again" (its "Login" section). For an OAuth entry that is `codex mcp login kagura-memory`, with `--scopes` set to the challenge's `scope`, comma-separated, after `insufficient_scope` (checked against codex-cli 0.145.0); a Bearer-key entry gets a new key in the variable its `bearer_token_env_var` names.
