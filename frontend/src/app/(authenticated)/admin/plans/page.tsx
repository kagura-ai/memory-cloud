...
import { CreditCard, Users, RefreshCw, Edit, CheckCircle, ChevronDown, Settings } from 'lucide-react';
...
export default function AdminPlansPage() {
  ...
  const [selectedWorkspace, setSelectedWorkspace] = useState<WorkspacePlan | null>(null);
  const [newPlan, setNewPlan] = useState<string>('');
  const [expandedWorkspaceId, setExpandedWorkspaceId] = useState<string | null>(null);
  const [quotaDetail, setQuotaDetail] = useState<WorkspaceQuotaDetail | null>(null);
  ...
  const toggleQuotaDetail = async (workspaceId: string) => {
    ...
    try {
      ...
      toast({ title: tCommon('error'), description: 'Failed to load quota details', variant: 'destructive' });
    } finally {
  ...
  const handleUpdateAddons = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to update addons';
        ...
        <TabsContent value="workspaces" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t('workspacesTable.description')}</CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                ...
                <p className="text-center text-muted-foreground py-12">{tCommon('noWorkspaces')}</p>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right">{t('workspacesTable.memories')}</TableHead>
                      <TableHead>{t('workspacesTable.usage')}</TableHead>
                      <TableHead className="text-right">{t('workspacesTable.actions')}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {workspaces.map((workspace) => {
                      const memoryPercentage = (workspace.total_memories / workspace.memory_limit) * 100;
                      ...
                      return (
                        ...
                        <TableRow className="cursor-pointer hover:bg-muted/50" onClick={() => toggleQuotaDetail(workspace.id)}>
                          <TableCell>
                          ...
                          <TableCell>
                            <PlanBadge planName={workspace.plan_name as 'free' | 'basic' | 'pro'} />
                          </TableCell>
                          <TableCell className="text-right">
                            {workspace.total_memories.toLocaleString()} / {workspace.memory_limit.toLocaleString()}
                          </TableCell>
                          <TableCell>
                            <Badge
                              variant={maxPercentage >= 95 ? 'destructive' : maxPercentage >= 80 ? 'secondary' : 'outline'}
                            >
                          ...
                          <TableCell className="text-right">
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={(e) => { e.stopPropagation(); openChangePlanDialog(workspace); }}
                            >
                        ...
                        {isExpanded && (
                          <TableRow>
                            <TableCell colSpan={5} className="bg-muted/30 p-0">
                              <div className="p-4">
                                {quotaLoading ? (
                                  <div className="flex justify-center py-4"><InlineSpinner size="sm" /></div>
                                ) : quotaDetail && quotaDetail.workspace_id === workspace.id ? (
                                  <div className="space-y-3">
                                    <div className="grid grid-cols-4 gap-2 text-xs font-medium text-muted-foreground border-b pb-2">
                                      <div>Resource</div>
                                      <div className="text-right">Base ({quotaDetail.plan_name})</div>
                                      <div className="text-right">Addon</div>
                                      <div className="text-right">Effective / Usage</div>
                                    </div>
                                    {[
                                      ...
                                      <div key={row.label} className="grid grid-cols-4 gap-2 text-sm">
                                        ...
                                        <div className="text-right text-muted-foreground">{row.base.toLocaleString()}</div>
                                        <div className="text-right">{row.addon > 0 ? <span className="text-green-600 dark:text-green-400">+{row.addon.toLocaleString()}</span> : <span className="text-muted-foreground">-</span>}</div>
                                        <div className="text-right">
                                          <span className="font-medium">{row.effective.toLocaleString()}</span>
                                          {row.usage !== null && <span className="text-muted-foreground"> / {row.usage.toLocaleString()} used</span>}
                                        </div>
                                      </div>
                                    ))}
                                    <div className="pt-2 border-t">
                                      <Button size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); openAddonDialog(); }}>
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  <TableRow>
                    <TableCell className="font-medium">{t('tiersTable.contextsPerWorkspace')}</TableCell>
                    <TableCell>1</TableCell>
                    <TableCell>3</TableCell>
                    <TableCell>30</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="font-medium">{t('tiersTable.memories')}</TableCell>
                    <TableCell>1,000</TableCell>
                    <TableCell>10,000</TableCell>
                    <TableCell>100,000</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="font-medium">{t('tiersTable.apiCalls')}</TableCell>
                    <TableCell>100</TableCell>
                    <TableCell>2,000</TableCell>
                    <TableCell>10,000</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="font-medium">{t('tiersTable.reranking')}</TableCell>
                    <TableCell>-</TableCell>
                    <TableCell>✅</TableCell>
                    <TableCell>✅</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="font-medium">{t('tiersTable.mcpAppCredentials')}</TableCell>
                    <TableCell>✅</TableCell>
                    <TableCell>✅</TableCell>
                    <TableCell>✅</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="font-medium">{t('tiersTable.memoryAgent')}</TableCell>
                    <TableCell>-</TableCell>
                    <TableCell>-</TableCell>
          ...
          <Card>
            ...
            <CardContent>
              {loading ? (
                ...
                <p className="text-center text-muted-foreground py-12">{tCommon('noChanges')}</p>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                    ...
                    {auditLog.map((entry) => (
                      <TableRow key={entry.id}>
                        ...
                        <TableCell>
                          {entry.old_plan && (
                            <>
                              <PlanBadge planName={entry.old_plan as 'free' | 'basic' | 'pro'} size="sm" />
                              <span className="mx-2">→</span>
                            </>
                          )}
                          <PlanBadge planName={entry.new_plan as 'free' | 'basic' | 'pro'} size="sm" />
                        </TableCell>
      ...
      <Dialog open={changePlanDialogOpen} onOpenChange={setChangePlanDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t('changePlanDialog.description')} <strong>{selectedWorkspace?.name}</strong>
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-4">
            <div>
              <label className="text-sm font-medium">{t('changePlanDialog.currentPlan')}</label>
              <div className="mt-2">
                {selectedWorkspace && <PlanBadge planName={selectedWorkspace.plan_name as 'free' | 'basic' | 'pro'} />}
              </div>
            </div>

            <div>
              <label className="text-sm font-medium">{t('changePlanDialog.newPlan')}</label>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleChangePlan} disabled={newPlan === selectedWorkspace?.plan_name}>
      ...
      <Dialog open={addonDialogOpen} onOpenChange={setAddonDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Adjust addon bonuses for <strong>{quotaDetail?.workspace_name}</strong>
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-xs text-muted-foreground whitespace-nowrap">
                  Effective: {((quotaDetail?.base.memory_limit ?? 0) + addonMemory).toLocaleString()}
                </span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                />
                <span className="text-xs text-muted-foreground whitespace-nowrap">
                  Effective: {((quotaDetail?.base.mcp_calls_per_day ?? 0) + addonMcp).toLocaleString()}
                </span>
              </div>
            </div>
