...
MCP tool (not read-only since #1683: a `query` runs recall, which writes Hebbian graph
updates — see [Tool annotations](../mcp-tools.md#tool-annotations); strict schema — `additionalProperties: false` is centrally
