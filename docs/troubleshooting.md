...
**If the entry points at the right server, sign the connection in again** — the server is fine. A wrong URL is a setup problem instead (`/kagura-memory:setup --check`).
...
- **Codex CLI:** `codex mcp login kagura-memory` for an OAuth entry (after `insufficient_scope`, add `--scopes` with the challenge's `scope`, comma-separated), or ask the kagura-memory skill to log in again.
...
2. **The MCP entry you edited is not the one in effect.** A `local`-scope entry (`~/.claude.json` → `projects[…].mcpServers`) silently shadows a `project` one of the same name. `claude mcp get kagura-memory` prints the winner's scope and URL; `claude mcp list` flags conflicting scopes. `claude mcp get` also prints configured headers and environment variables with their values, so pipe it through `sed -E 's/^(    [A-Za-z0-9_-]+)([:=]).*/\1\2 <redacted>/'` before pasting the output anywhere. An entry whose command is `kagura-mcp` (written by `kagura setup claude --profile <name>`) has no URL of its own: it forwards to its `--server` argument, or else to the CLI profile's server + `/mcp` (`kagura auth list`), and from `kagura-mcp` 0.39.0 its `--guardrails <off|context-id>` and `--tool-profile <name>` arguments set `?guardrails=` and `?profile=` on that URL, replacing any value `--server` carries. So an entry with `--guardrails off` is already in the hooks lane; before 0.39.0 the query can only come from `--server`.
