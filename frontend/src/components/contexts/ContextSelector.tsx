      ...
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          <span className="max-w-[150px] truncate font-medium">
            {displayContext?.display_name || displayContext?.name || t('noContext')}
          </span>
          ...
          contexts.map((context) => (
            <DropdownMenuItem
              ...
              className={cn(
                ...
                context.id === effectiveContextId && 'bg-slate-100 dark:bg-slate-800'
              )}
            >
              <div className="flex flex-col">
                <div className="flex items-center gap-1.5">
                  {context.is_public ? (
                    <span className="text-xs" title={t('public')}>🌍</span>
                  ) : context.is_private ? (
                    <span className="text-xs" title={t('private')}>🔒</span>
                  ) : (
                    <span className="text-xs" title={t('shared')}>👥</span>
                  )}
                  <span className="font-medium">{context.display_name || context.name}</span>
                </div>
                {context.is_default && (
              ...
              {context.id === effectiveContextId && (
                <span className="text-xs font-medium text-brand-green-600">{t('current')}</span>
              )}
            </DropdownMenuItem>
          ))
        )}
