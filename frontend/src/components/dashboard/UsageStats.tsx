...
import { Brain, Database, Zap, TrendingUp, AlertTriangle, XCircle } from 'lucide-react';
...
export const UsageStats = forwardRef<UsageStatsRef, UsageStatsProps>(
  ...
  const [currentUsage, setCurrentUsage] = useState<UsageCurrentData | null>(null);
  ...
  const fetchAllStats = async () => {
    try {
      ...
      const [current, hist, brkdwn] = scope === 'workspace'
        ? await Promise.all([
            getWorkspaceUsageCurrent(),
        ...
        : await Promise.all([
            ...
            apiClient.get<UsageBreakdown>('/api/v1/usage/breakdown?days=30'),
          ]);
  ...
  useEffect(() => {
    ...
    const shouldFetch = scope === 'workspace'
    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-2xl font-bold mb-4">
          {scope === 'workspace' ? t('usage') : t('currentUsage')} - {currentUsage.plan.plan_name.toUpperCase()} {t('plan')}
        </h2>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold mb-2">
                {currentUsage.usage.memory_count} / {currentUsage.plan.memory_limit}
              </div>
              <Progress
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {t('percentUsed', { percent: currentUsage.memory_usage.percentage.toFixed(1) })}
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold mb-2">
                {currentUsage.usage.api_calls_today} / {currentUsage.plan.daily_api_limit}
              </div>
              <Progress
              ...
              />
              <p className="text-xs text-muted-foreground mt-2">
                {t('percentUsed', { percent: currentUsage.daily_api_usage.percentage.toFixed(1) })}
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold mb-2">
                {currentUsage.usage.api_calls_this_week} / {currentUsage.plan.weekly_api_limit}
              </div>
              <Progress
                value={Math.min(currentUsage.weekly_api_usage.percentage, 100)}
                className="h-2"
              />
              <p className="text-xs text-muted-foreground mt-2">
                {t('percentUsed', { percent: currentUsage.weekly_api_usage.percentage.toFixed(1) })}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <div className="p-3 border rounded-lg bg-blue-50 dark:bg-blue-950/20">
              <div className="text-xs text-gray-600 dark:text-gray-400 mb-1">MCP Calls</div>
              <div className="text-lg font-bold text-blue-600">{currentUsage.usage.mcp_calls_today}</div>
            ...
            <div className="p-3 border rounded-lg bg-green-50 dark:bg-green-950/20">
              <div className="text-xs text-gray-600 dark:text-gray-400 mb-1">REST API</div>
              <div className="text-lg font-bold text-green-600">{currentUsage.usage.rest_calls_today}</div>
              <div className="text-xs text-gray-500 dark:text-gray-400">
                {currentUsage.usage.api_calls_today > 0
            ...
            <div className="p-3 border rounded-lg bg-purple-50 dark:bg-purple-950/20">
              <div className="text-xs text-gray-600 dark:text-gray-400 mb-1">Public API</div>
              <div className="text-lg font-bold text-purple-600">{currentUsage.usage.public_calls_today}</div>
              <div className="text-xs text-gray-500 dark:text-gray-400">
                {currentUsage.usage.api_calls_today > 0
            ...
            <div className="h-64">
              {breakdown.by_endpoint.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <RechartsPie>
                    <Pie
                      ...
                      cy="50%"
                      labelLine={false}
                      label={(entry: any) => `${entry.endpoint}: ${entry.percentage.toFixed(1)}%`}
                      outerRadius={80}
                      dataKey="count"
                    >
                ...
                <div className="flex items-center justify-center h-full">
                  <p className="text-sm text-gray-500 dark:text-gray-400">{t('noUsageData')}</p>
                </div>
              )}
            </div>
          </CardContent>
                ...
                breakdown.by_endpoint.map((ep, index) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400">{ep.percentage.toFixed(1)}%</div>
                    </div>
                  </div>
                ))
              ) : (
