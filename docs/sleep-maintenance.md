...
- **Direct-score eligibility** (#1229, #1231): a merge decision only executes when the (winner, loser) pair's **direct** pairwise similarity is at/above the dedup threshold. Union-Find clusters chain transitively, so the judge can otherwise nominate two members whose true pairwise similarity is far below the threshold (observed 0.43–0.66 under 0.92) — merging distinct facts that merely share a template. When the pair is missing from this run's candidate list (the per-memory top-10 neighbor search saturates in dense duplicate clusters, #1231), the direct cosine is computed on demand from the summary vectors already embedded during candidate discovery — a genuinely duplicate pair merges instead of being false-positive vetoed. Report counters: `llm_merge_guarded` (sub-threshold vetoes only), `llm_merge_rescued` (missing-edge pairs whose on-demand score met the threshold), `llm_merge_unverifiable` (no score computable — vector unavailable; fail-closed veto).
