"""PreToolUse latency probe for the Claude guardrail hook (#1619).

Skipped unless ``KAGURA_HOOK_BENCH=1`` (CI runners are noisy). Runs the exact
``hooks.json`` command 200 times over a 50-entry cache with realistic commands
and asserts the script-alone p95 stays at or below 50 ms, the merge gate from
the issue. The whole-run number (sh guard + interpreter) is reported without a
target.
"""
...
from tests.plugin.conftest import HOOK_SCRIPT, PluginEnv, bash_pre, command_for, item
...
def _cache_items() -> list[dict]:
    ...
    items = []
    for n in range(50):
        match, action = patterns[n % len(patterns)]
        tool = "Bash|PowerShell" if n % 5 else "Edit|Write"
        items.append(
            item(
                n + 1,
...
def test_pre_tool_use_p95(plugin_env: PluginEnv) -> None:
    plugin_env.write_cache(_cache_items())
    command = command_for("PreToolUse")
    script_ms: list[float] = []
    whole_ms: list[float] = []
    for i in range(RUNS):
        body = json.dumps(bash_pre(COMMANDS[i % len(COMMANDS)])).encode()
        started = time.perf_counter()
        proc = subprocess.run(
            ["python3", "-I", "-S", str(HOOK_SCRIPT), "--client", "claude"],
            input=body,
            capture_output=True,
