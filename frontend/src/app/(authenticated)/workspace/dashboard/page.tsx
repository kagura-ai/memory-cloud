...
import { RefreshCw, Database, HardDrive, FolderOpen, AlertCircle, Lock, Users, Download, ArrowUpDown, Calendar } from 'lucide-react';
...
import { getContextStats, ContextStatsResponse, getWorkspaceMemoryTimeline, MemoryTimelineResponse, getContextUserActivity, ContextUserActivityResponse, getWorkspaceMemberUsage, MemberUsageEntry } from '@/lib/api/workspaces';
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">{t('memories', { default: 'Memories' })}</TableHead>
              <TableHead className="text-right">{t('apiToday', { default: 'API Today' })}</TableHead>
              <TableHead className="text-right">{t('apiWeek', { default: 'API Week' })}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {members.map((m) => (
              <TableRow key={m.user_id}>
                ...
                <TableCell className="text-right">{m.memory_count.toLocaleString()}</TableCell>
                <TableCell className="text-right">{m.api_calls_today.toLocaleString()}</TableCell>
                <TableCell className="text-right">{m.api_calls_week.toLocaleString()}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
...
export default function WorkspaceStatsPage() {
  ...
  const [contextStats, setContextStats] = useState<ContextStatsResponse | null>(null);
  const [memoryTimeline, setMemoryTimeline] = useState<MemoryTimelineResponse | null>(null);
  ...
  const [sortBy, setSortBy] = useState<'name' | 'memory' | 'activity'>('memory');
  ...
  const fetchStats = async () => {
    try {
      setLoading(true);
      setError(null);
      const [statsResponse, contextStatsResponse, timelineResponse] = await Promise.all([
        apiClient.get<WorkspaceStats>('/api/v1/workspace/stats'),
        currentWorkspaceId ? getContextStats(currentWorkspaceId) : Promise.resolve(null),
        currentWorkspaceId ? getWorkspaceMemoryTimeline(currentWorkspaceId, timelineDays) : Promise.resolve(null)
      ]);
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Context Name', 'Memory Count', 'Last Activity', 'Members'];
  ...
  useEffect(() => {
    const isAdmin = currentWorkspace?.current_user_role === 'admin' || currentWorkspace?.current_user_role === 'owner';
  ...
  }, [selectedContextId, activityDays, currentWorkspaceId, currentWorkspace?.current_user_role]);
  ...
  useEffect(() => {
    if (stats?.contexts && stats.contexts.length > 0 && !selectedContextId && !hasSetDefaultContext.current) {
  ...
  const sortedContexts = stats?.contexts ? [...stats.contexts].sort((a, b) => {
    const aDetail = contextStats?.contexts.find(c => c.context_id === a.context_id);
    const bDetail = contextStats?.contexts.find(c => c.context_id === b.context_id);
    ...
    switch (sortBy) {
      ...
      case 'memory':
        ...
        break;
      case 'activity':
        aValue = aDetail?.last_activity ? new Date(aDetail.last_activity).getTime() : 0;
        bValue = bDetail?.last_activity ? new Date(bDetail.last_activity).getTime() : 0;
        break;
    }
    ...
    <PageContainer>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          {stats && <PlanBadge planName={stats.plan_name as 'free' | 'basic' | 'pro'} size="lg" />}
        </div>
      ...
      {loading && !stats ? (
        ...
        <>
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    >
                      <div className="flex items-center justify-end gap-1">
                        {t('lastActivity')}
                        {sortBy === 'activity' && <ArrowUpDown className="h-3 w-3" />}
                      </div>
                    </TableHead>
                    <TableHead className="text-right">{t('apiCallsWeek')}</TableHead>
                    <TableHead className="text-right">{t('activeUsersWeek')}</TableHead>
                    <TableHead className="text-right">{t('members')}</TableHead>
                    <TableHead className="text-right">{t('percentOfTotal')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {stats.contexts.length === 0 && !stats.private_aggregation ? (
                    <TableRow>
                    ...
                    <>
                      ...
                      {sortedContexts.map((context) => {
                        const percentage = stats.total_memories > 0
                          ? ((context.memory_count / stats.total_memories) * 100).toFixed(1)
                          : '0.0';
                        const contextDetail = contextStats?.contexts.find(c => c.context_id === context.context_id);
                        return (
                          <TableRow key={context.context_id}>
                            <TableCell className="font-medium">
                              <div className="flex items-center gap-2">
                                {context.is_private ? (
                                  <Lock className="h-3 w-3 text-gray-400" aria-label={t('privateContext')} role="img" />
                                ) : (
                                  <Users className="h-3 w-3 text-blue-500" aria-label={t('sharedContext')} role="img" />
                                )}
                                <Link
                            ...
                            <TableCell className="text-sm text-gray-600 dark:text-gray-400">
                              {context.created_by_name || context.created_by || t('notAvailable')}
                            </TableCell>
                            <TableCell className="text-right">{context.memory_count.toLocaleString()}</TableCell>
                            <TableCell className="text-right text-sm text-gray-500">
                              {contextDetail?.last_activity
                                ? formatDistanceToNow(new Date(contextDetail.last_activity), { addSuffix: true })
                                : 'Never'}
                            </TableCell>
                            <TableCell className="text-right">
                              <span className={contextDetail?.api_calls_week && contextDetail.api_calls_week > 0 ? 'text-green-600 font-medium' : 'text-gray-400'}>
                                {contextDetail?.api_calls_week?.toLocaleString() || '0'}
                              </span>
                            </TableCell>
                            <TableCell className="text-right">
                              <span className={contextDetail?.active_users_week && contextDetail.active_users_week > 0 ? 'text-blue-600 font-medium' : 'text-gray-400'}>
                                {contextDetail?.active_users_week || '0'}
                              </span>
                            </TableCell>
                            <TableCell className="text-right">{contextDetail?.member_count || '-'}</TableCell>
                      ...
                      {stats.private_aggregation && stats.private_aggregation.context_count > 0 && (
                        <TableRow className="bg-gray-50 dark:bg-gray-800/50">
                          <TableCell className="font-medium text-gray-600 dark:text-gray-400">
                            <div className="flex items-center gap-2">
                              <Lock className="h-3 w-3" aria-label={t('privateContext')} role="img" />
                              <span className="italic">
                                {t('othersPrivate', { count: stats.private_aggregation.context_count })}
                              </span>
                            </div>
                          </TableCell>
                          <TableCell className="text-sm text-gray-500 dark:text-gray-500 italic">
                            <div className="flex items-center gap-1">
                              <Lock className="h-3 w-3" aria-label={t('hidden')} role="img" />
                              <span>{t('hidden')}</span>
                            </div>
                          </TableCell>
                          ...
                          <TableCell className="text-right text-gray-600 dark:text-gray-400">
                            {stats.total_memories > 0
                              ? ((stats.private_aggregation.memory_count / stats.total_memories) * 100).toFixed(1)
                              : '0.0'}%
                          </TableCell>
                        </TableRow>
                      )}
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  <Button variant={timelineDays === 7 ? 'default' : 'outline'} size="sm" onClick={() => setTimelineDays(7)}>
                    7 {t('days')}
                  </Button>
                  <Button variant={timelineDays === 30 ? 'default' : 'outline'} size="sm" onClick={() => setTimelineDays(30)}>
                    30 {t('days')}
                  </Button>
                </div>
              </div>
          ...
          {(currentWorkspace?.current_user_role === 'admin' || currentWorkspace?.current_user_role === 'owner') && stats?.contexts && stats.contexts.length > 0 && (
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant={activityDays === 7 ? 'default' : 'outline'} size="sm" onClick={() => setActivityDays(7)}>
                    7 {t('days')}
                  </Button>
                  <Button variant={activityDays === 30 ? 'default' : 'outline'} size="sm" onClick={() => setActivityDays(30)}>
                    30 {t('days')}
                  </Button>
                </div>
              </div>
                ...
                <CardContent className="pt-6">
                  {activityError ? (
                        ...
                        {userActivity.users.length === 0 ? (
                          ...
                          userActivity.users.map((user) => (
                            <TableRow key={user.user_id}>
                              ...
                              <TableCell className="text-right">
                                <span className={user.api_calls > 0 ? 'text-green-600 font-medium' : 'text-gray-400'}>
                    ...
                    <div className="flex items-center justify-center py-8">
                      ...
                      <span className="ml-3 text-slate-500">{t('loadingUserActivity')}</span>
                    </div>
                  )}
                </CardContent>
              </Card>
