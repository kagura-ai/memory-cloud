...
import { cn, typography, colors, spacing, transitions } from '@/styles/design-tokens';
...
interface NavItem {
  ...
  requiredWorkspaceRole?: 'owner' | 'admin';  // Workspace role (owner/admin can access)
...
export function Sidebar() {
  ...
  const [collapsedSections, setCollapsedSections] = useState<Record<string, boolean>>(() => {
    ...
    navigationGroups.forEach(group => {
      if (group.collapsible) {
        initial[group.titleKey] = saved[group.titleKey] ?? (group.defaultCollapsed || false);
      }
  ...
  const getItemName = (item: NavItem): string => {
    ...
    if (item.showMemberCount && currentWorkspace?.member_count && currentWorkspace.member_count >= 2) {
      ...
      <nav className="flex-1 px-4 py-3 space-y-4 overflow-y-auto">
        {navigationGroups.map((group) => {
          ...
          const visibleItems = group.items.filter((item) => {
            ...
            // Check workspace role (owner/admin only)
            if (item.requiredWorkspaceRole) {
              const userWorkspaceRole = currentWorkspace?.current_user_role;
              if (item.requiredWorkspaceRole === 'admin') {
                // Admin or Owner can access
                return userWorkspaceRole === 'owner' || userWorkspaceRole === 'admin';
              } else if (item.requiredWorkspaceRole === 'owner') {
          ...
          const isCollapsed = group.collapsible && collapsedSections[group.titleKey];
                ...
                {visibleItems.map((item) => {
                  ...
                  if (item.children) {
                    ...
                    const visibleChildren = item.children.filter((child) => {
                      if (child.requiredRole && !hasRole(user, child.requiredRole)) {
                        return false;
                      }
                      if (child.requiredWorkspaceRole) {
                        const userWorkspaceRole = currentWorkspace?.current_user_role;
                        if (child.requiredWorkspaceRole === 'admin') {
                          return userWorkspaceRole === 'owner' || userWorkspaceRole === 'admin';
                        } else if (child.requiredWorkspaceRole === 'owner') {
                          return userWorkspaceRole === 'owner';
                        }
                      }
                      return true;
                      ...
                      {item.nameKey === 'contexts' && (contextCount === 0 || contextCount === null) && (
                        <span title={contextCount === null ? 'Failed to load contexts' : 'No contexts found'}>
                          <AlertTriangle
                            ...
                            aria-label={contextCount === null ? 'Failed to load contexts' : 'No contexts found'}
                          />
                        </span>
                      )}
                    </Link>
        ...
        {user && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className={cn(
                ...
                <div className="flex-1 text-left overflow-hidden">
                  <p className={cn(typography.bodySmall, 'font-semibold truncate')}>
                    {user.name}
                  </p>
                  {user.role === 'admin' && (
                    <p className={cn(typography.caption, 'text-red-600 dark:text-red-400 font-medium')}>
                      {t('systemAdmin')}
                    </p>
                  )}
                </div>
            ...
            <DropdownMenuContent className="w-56" align="end" side="top">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => { router.push('/profile'); setIsOpen(false); }}>
