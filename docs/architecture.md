...
**MCP duplicate contract (#1321)**: the sticky-upsert above protects `origin`, but not a declared edge's *values* — so the MCP `create_edge` handler adds its own layer. On an existing (source, target) pair: an auto edge (`hebbian`/`semantic`) is upgraded and the response reports `operation: "updated"` with a `previous` pre-image; a `declared` edge with identical values is a no-op (`operation: "unchanged"`, keeps retries idempotent); a `declared` edge with different values is rejected with `edge_exists` unless the caller passes `overwrite=true` — declared links are provenance and are never silently clobbered. This contract is MCP-only: the repository upsert stays unconditional because SDK/worker replay paths depend on it.
