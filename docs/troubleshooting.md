...
2. **The MCP entry you edited is not the one in effect.** A `local`-scope entry (`~/.claude.json` → `projects[…].mcpServers`) silently shadows a `project` one of the same name. `claude mcp get kagura-memory` prints the winner's scope and URL; `claude mcp list` flags conflicting scopes. `claude mcp get` also prints configured headers with their values, so pipe it through `sed -E 's/^(    [A-Za-z0-9_-]+:).*/\1 <redacted>/'` before pasting the output anywhere.
